...
const MapTest2 = ({ watchLat1, watchLng1 }) => {
    ...
    useEffect(() => {
        if (map !== null) {
            ...
            // console.log(map._status.center);
            map.setCenter(new window.Tmapv2.LatLng(watchLat1, watchLng1));

            if (marker) {
                marker.setPosition(
                    new window.Tmapv2.LatLng(watchLat1, watchLng1),
                );
            } else {
                // 새 마커 생성 및 지도에 추가
                const newMarker = new window.Tmapv2.Marker({
                    position: new window.Tmapv2.LatLng(watchLat1, watchLng1),
                    map: map,
                });
    ...
    const initTmap = () => {
        ...
        const newMap = new window.Tmapv2.Map("map_div", {
            center: new window.Tmapv2.LatLng(watchLat1, watchLng1),
            width: "100%",
            height: "400px",
            zoom: 15,
        });
